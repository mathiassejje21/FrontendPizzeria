...
export async function mostrarDetalleCarrito() {
  ...
  async function hundleCheckout() {
    if (userSession) {
      ...
      const repartidorId = 
        metodo === 1 ? null : parseInt(document.getElementById("select-repartidor")?.value,10) || null;
      ...
      const pedido = {
        ...
        detalles: carrito.map((p) => {
          ...
          if (p.personalizaciones?.id_tamano) {
            personalizaciones.id_tamano = p.personalizaciones.id_tamano;
          }

          if (Array.isArray(p.personalizaciones?.ingredientes) && 
              p.personalizaciones.ingredientes.length > 0) {
            personalizaciones.ingredientes = p.personalizaciones.ingredientes;
          }
        ...
        <table class="table table-striped mt-3">
          ...
          <tbody>
            ${carrito.map(p => {
              ...
              const precioUnit =
                (Number(p.precioReal) * (p.tamanio?.factor_precio ?? 1)) +
                extraIng;
      ...
      <div class="carrito-side ${!userSession ? "hidden" : ""}">
          <div class="side-section">
            ...
            <select class="form-select" id="select-pago"
              @change=${() => {
                const metodo = Number(document.getElementById("select-pago").value);
                document.getElementById("box-repartidor").classList.toggle("hidden", metodo === 1);
              }}>

              ${metodoPago.map(m => html`
  ...
  document.getElementById("box-repartidor")?.classList.toggle("hidden", metodoInicial === 1);
}
